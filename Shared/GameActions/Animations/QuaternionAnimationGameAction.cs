...
namespace WaveEngine.Components.GameActions
{
    ...
    public class QuaternionAnimationGameAction : UpdatableGameAction
    {
        ...
        public Quaternion BounceInOutEase(float t, Quaternion b, Quaternion c, float d)
        {
            if (t < d / 2)
            {
                return (this.BounceInEase(t * 2, Quaternion.Identity, c, d) * .5f) + b;
            }
            else
            {
                return (this.BounceOutEase((t * 2) - d, Quaternion.Identity, c, d) * .5f) + (c * .5f) + b;
            }
        }
        ...
        public Quaternion ElasticInOutEase(float t, Quaternion b, Quaternion c, float d)
        {
            ...
            if (t < 1)
            {
                return ((a * (float)Math.Pow(2, 10 * (t -= 1)) * (float)Math.Sin(((t * d) - s) * (2 * (float)Math.PI) / p)) * -0.5f) + b;
            }
